...
class SchemaAgent:
    ...
    def __init__(self, server=None, database=None, source_type="SQL Server", oracle_config=None):
        ...
        Args:
            ...
            source_type: "SQL Server", "Oracle (Real)", or "Oracle (Simulator)"
    ...
    def discover_schema(self, schema_owner=None):
        ...
        elif self.source_type == "Oracle (Simulator)":
            return self._discover_oracle_simulator_schema()
        else:
    ...
    # ============================================================
    # Oracle Simulator Schema Discovery
    # ============================================================

    def _discover_oracle_simulator_schema(self):
        """
        Load schema from oracle_metadata.json and return
        standardized DataFrame matching the SQL Server format.
        """
        data_dir = os.path.abspath(
            os.path.join(
        ...
        metadata_file = os.path.join(data_dir, "oracle_metadata.json")

        if not os.path.exists(metadata_file):
            raise FileNotFoundError(
                f"Oracle metadata file not found: {metadata_file}"
            )
        ...
        # Export metadata to JSON (same format as SQL Server)
        self._export_oracle_metadata(df)
    ...
    def discover_tables(self):
        ...
        if self.source_type == "SQL Server":
            ...
            return self._discover_oracle_simulator_tables()
        else:
            raise ValueError(f"Unsupported source type: {self.source_type}")
    ...
    def _discover_oracle_simulator_tables(self):
        """Get tables from Oracle simulator metadata."""
        data_dir = os.path.abspath(
            os.path.join(
                os.path.dirname(__file__),
                "..",
