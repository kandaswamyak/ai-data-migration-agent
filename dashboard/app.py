...
if st.session_state["active_tab"] == 0:
    ...
    with col_src:
        source_type = st.selectbox("Select Source Database", ["SQL Server", "Oracle (Real)", "Azure SQL", "Oracle (Simulator)"], key="src_sel")
    with col_tgt:
        target_type = st.selectbox("Select Target Database", ["Azure SQL", "SQL Server", "Oracle"], key="tgt_sel")

    # Store target selection in session state
    st.session_state["target_type"] = target_type
    st.info(f"📌 Migration path: **{source_type}** → **{target_type}**")
    st.divider()
    ...
    if source_type == "Oracle (Real)":
        ...
        c1, c2 = st.columns(2)
        with c1:
            azure_server = st.text_input("Server", value=os.getenv("AZURE_SQL_SERVER", ""), key="t1_az_srv",
                                         placeholder="myserver.database.windows.net")
            azure_user = st.text_input("Username", value=os.getenv("AZURE_SQL_USERNAME", ""), key="t1_az_user")
        with c2:
            azure_db = st.text_input("Database", value=os.getenv("AZURE_SQL_DATABASE", ""), key="t1_az_db")
            azure_pass = st.text_input("Password", type="password", value=os.getenv("AZURE_SQL_PASSWORD", ""), key="t1_az_pass")

        if st.button("Connect to Azure SQL", key="t1_az_connect"):
            try:
                from connectors.azuresql_connector import AzureSQLConnector
                with st.spinner("Connecting to Azure SQL..."):
                    connector = AzureSQLConnector(azure_server, azure_db, azure_user, azure_pass)
                    connector.connect()
                    tables = connector.discover_tables()
                    connector.disconnect()
                    clear_downstream_state()
                    st.session_state["source_type"] = "Azure SQL"
                    st.session_state["azure_src_server"] = azure_server
                    st.session_state["azure_src_db"] = azure_db
                    st.session_state["azure_src_user"] = azure_user
                    st.session_state["azure_src_pass"] = azure_pass
                    st.success(f"✅ Connected to Azure SQL! Found {len(tables)} tables.")
            except Exception as e:
                st.error(f"Azure SQL connection failed: {str(e)[:200]}")
...
# ============================================================
    ...
    if "schema" not in st.session_state:
        ...
        st.write(f"**Source:** {st.session_state.get('source_type','')} → **Target:** {st.session_state.get('target_type', 'Azure SQL')}")
...
# ============================================================
    ...
    if "migration_executed" not in st.session_state:
        ...
        st.info("Migration executed. Running validation checks...")

        # Use real validation results if available, otherwise generate sample checks
        checks = st.session_state.get("validation_results", [])
        if not checks:
            checks = [
            ...
            st.session_state["validation_results"] = checks

        # Display each check with correct PASS/FAIL indicators
        for ck in checks:
            ...
            if ck["status"] == "PASS":
                c4.write(f"✅ {ck['status']}")
            else:
                c4.write(f"❌ {ck['status']}")

        # Calculate actual pass/fail counts
        pass_count = sum(1 for ck in checks if ck["status"] == "PASS")
        ...
        if ai and "validation_ai_analysis" not in st.session_state:
            with st.spinner("AI interpreting results..."):
                try:
                    prompt = f"Briefly interpret these migration validation results ({pass_count} passed, {fail_count} failed): {json.dumps(checks)}"
                    st.session_state["validation_ai_analysis"] = ai.call(prompt, "You are a data validation expert.")
                except Exception:
                    st.session_state["validation_ai_analysis"] = f"{pass_count} checks passed, {fail_count} failed." if fail_count > 0 else "All validation checks passed. Data integrity confirmed."
        ...
        st.divider()
        if all_passed:
            st.success(f"✅ ALL {pass_count} VALIDATIONS PASSED")
        else:
            st.error(f"❌ {fail_count} VALIDATION(S) FAILED — {pass_count} passed, {fail_count} failed")
        st.session_state["validation_complete"] = True
        st.session_state["validation_all_passed"] = all_passed
        st.session_state["validation_pass_count"] = pass_count
        st.session_state["validation_fail_count"] = fail_count
...
# ============================================================
    ...
    if "validation_complete" not in st.session_state:
        ...
        all_passed = st.session_state.get("validation_all_passed", True)
        pass_count = st.session_state.get("validation_pass_count", 0)
        fail_count = st.session_state.get("validation_fail_count", 0)

        # Derive actual statuses
        migration_status = "SUCCESS" if all_passed else "ISSUES FOUND"
        validation_status = "PASSED" if all_passed else f"FAILED ({fail_count})"

        c1,c2,c3,c4 = st.columns(4)
        c1.metric("Tables", len(tables))
        c2.metric("Columns", len(mappings))
        c3.metric("Status", migration_status)
        c4.metric("Validation", validation_status)

        # Show warning if failures exist
        if not all_passed:
            st.error(f"⚠️ {fail_count} validation check(s) failed. Review the Validation tab for details.")
        else:
            ...
            if "executive_summary" not in st.session_state:
                with st.spinner("Generating summary..."):
                    ...
                    ctx = {"source_type": st.session_state.get("source_type", "Oracle"),
                           "target_type": st.session_state.get("target_type", "Azure SQL"),
                           "mappings": mappings,
        ...
        report_data = {
            ...
            "target": st.session_state.get("target_type", "Azure SQL"),
            "tables": len(tables),
